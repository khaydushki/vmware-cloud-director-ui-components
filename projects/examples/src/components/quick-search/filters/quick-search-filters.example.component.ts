...
export class QuickSearchFiltersExampleComponent implements OnInit, OnDestroy {
    formGroup: FormGroup;
    kbdShortcut = 'mod+f';
    ...
    filters: QuickSearchFilter[] = [
        {
            id: 'type',
            options: [{ display: 'action' }, { display: 'other' }],
        },
    ];
    ...
    ngOnInit(): void {
        ...
        this.searchRegistrar.register({
            order: 1,
            sectionName: 'Super Section',
            children: [this.actionsSearchProvider, new PagedActionsSearchProvider()],
        });
